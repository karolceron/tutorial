---
title: 'Boas práticas na análise de séries temporais em herpetologia'
author: "Karoline Ceron, Marcos R. Severgnini & Diogo B. Provete"
date: "08/06/2020"
...
Caso você não tenha eles instalados, instale-os com a função *install.packages(nome do pacote)* e depois carregue eles.
```{r setup, include=FALSE}
...
Agora, vamos importar os dados de demonstração! 
```{r}
dados<-read.table("data.txt", header=T) #dados empíricos de uma comunidade de anuros no Parque Estadual da Serra Furada, SC.
dados
```
MARCOS > Antes de realizar análises para verificar a influência das variáveis preditoras sobre a riqueza e abundância dos anuros, precisamos padronizar os dados, pois dados de precipitação, temperatura e umidade possuem diferentes unidades de medida (C°, %, mm). dados_pad = dados padronizados
```{r}
dados_pad <- decostand(dados[,c(2,3,4)], "standardize")
```
MARCOS : Agora precisamos substituir os dados que estão na tabela pelos que foram padronizados
```{r}
dados[,2] <- dados_pad[,1]
dados[,3] <- dados_pad[,2]
dados[,4] <- dados_pad[,3]
...
shapiro.test(dados$riqueza) 
hist(dados$riqueza)
```

A normalidade é alcançada com um valor de p < 0,05
...
Nossos dados não apresentam distribuíção normal, neste caso temos que aplicar testes NÃO-PARAMÉTRICO (veja a FIG. XX do manuscrito)

Vamos começar realizando uma **CORRELAÇÃO DE SPEARMAN**. Os valores das correlações variam entre -1 até 1. A correlação 1 indica uma perfeita relação linear positiva entre as variáveis (quando uma variável aumenta a outra aumenta na mesma direção)
a correlação -1 indica uma perfeita associação linear negativa (quando uma variável aumenta a outra diminui) 

Vamos testar a correlação entre abundância dos anuros e a precipitação durante 12 meses. A variável resposta é a segunda e a variável preditora vem antes.
```{r}
cor.test(dados$precipitacao,dados$abundancia,  method = "spearm")
```

*p* não significativo (p>0.05), ou seja, não há uma correlação entre as variáveis


Vamos testar agora a abundância dos anuros e a temperatura
```{r}
cor.test(dados$temperatura, dados$abundancia, method = "spearm")
```

*p* marginalmente significativo (p = 0.04), ou seja, há uma correlação entre as variáveis
coeficiente (rho) = 0.59, ou seja, correlação positiva.

Vamos visualizar essa relação
```{r}
plot(dados$temperatura, dados$abundancia, xlab = "Temperatura °C", ylab = "Abundância de anuros")
```
...
MARCOS > Novamente devemos padronizar os dados que possuem diferentes unidades de medida como no exemplo anterior. dados_pad2 = dados padronizados
```{r}
dados_pad2 <- decostand(dados_normal[,c(2,3,4)], "standardize")
```

MARCOS > Substituindo os dados padronizados na tabela 
```{r}
dados_normal[,2] <- dados_pad[,1]
dados_normal[,3] <- dados_pad[,2]
dados_normal[,4] <- dados_pad[,3]
head(dados_normal)
```

Vamos testar novamente a normalidade dos dados
```{r}
shapiro.test(dados_normal$abundancia) 
...
A partir disso podemos realizar a **CORRELAÇÃO DE PEARSON**, para dados não-paramétricos
```{r}
cor.test(dados_normal$temperatura, dados_normal$abundancia, method = "pearson")
```

Há correlação
Coeficiente (cor) = 0.59, ou seja, correlação positiva.


**No entanto, como você leu no artigo a correlação não é recomendada para dados temporais, pois geralmente estamos lidando com dados autocorrelacionados**


Para lidar com isso, vamos partir para as análises de **REGRESSÃO**
...
Ele indica que há relação entre as variáveis testadas

No entanto, vamos checar os resíduos da regressão para ser se o modelo está adequado com nossos dados
```{r}
plot(rstandard(mod.ols) ~ dados_normal$mes)
abline(h = 0, lty = 2)
lines(rstandard(mod.ols) ~ dados_normal$mes)
```

Há um claro padrão temporal nos resíduos. Isso indica que nossa variável apresenta correlação temporal.
...
Caso não haja autocorrelação, os valores de p seriam >0.5 (MARCOS > AQUI SERIA >0.5 mesmo ou <0.5?)
Isso indica que o modelo linear não é adequado para nossos dados

##**Regressão dos mínimos quadrados generalizados (GLS)**

Vamos criar um GLS sem controlar nada, que é basicamente a regressão linear feita anteriormente
```{r}
mod.gls <- gls(abundancia ~ temperatura, data=dados_normal)
```

Agora um modelo GLS indicando que nossos dados são distribuídos temporalmente. Aqui indicamos a variável de correlação (mês das coletas), indicando que os dados são correlacionados temporalmente e que a lag são os meses

MARCOS > Karol na análise anterior vc tinha usado o conjunto de dados "dados_normal" e nos seguintes usou o "dados" que representa o conjunto de distribuição não normal e comparou os modelos. Não sei se foi proposital, mas tomei a liberdade de padronizar para dados_normal para estes modelos. Note que alguns valores mudaram.
```{r}
mod.gls1 <- gls(abundancia ~ temperatura, data=dados_normal, correlation=corAR1(form=~mes))
```

Você pode testar as várias opcões de correlações existentes nesta função
Veja o help da função para maiores detalhes 
```{r}
?gls
```
...
Agora vamos verificar se a adição desta informação de correlação resultou em resultados diferentes entre os modelos 0 e 1
```{r}
anova(mod.gls, mod.gls1)
```

Sim, eles são diferentes (p<0.05)

Agora entre o modelo 1 e 2
```{r}
anova(mod.gls2, mod.gls1)
```
Não apresentam diferença estatística significante

Agora vamos usar a **seleção de modelos** para ver qual se adequa melhor aos nossos dados 
```{r}
AICctab(mod.gls, mod.gls1, mod.gls2,base=T, weights=T)
```
...
A linha azul significa que as médias das autocorrelações excederam os limites de confiança. Este modelo ainda não é adequado para nossos dados. Os resíduos ainda apresentam um padrão temporal

**GAM (modelos aditivos generalizados)**

MARCOS > Vamos iniciar o uso do GAM pelo modelo mais simples e ver se ele resolve o problema de autocorrelação dos resíduos

```{r}
gam1 <- gam(abundancia ~ temperatura + s(mes), data = dados, family = poisson)
```

Otendo o resulta do modelo
```{r}
summary(gam1)
```
...
gam.check(gam1)
```
Os resíduos melhoraram em relação ao GLS, porém sem um parâmetros para controlar os efeitos de correlação, as estimativas do modelo não são confiáveis
...
MARCOS > aqui tbm colquei o gam.check para visiualizar graficamente a distribuição dos redíduos em relação a um preditor linear. Ele ajuda a verificar um dos pressupostos do modelo que é homogeneidade. 
```{r}
acf(residuals(gamm1$gam))
gam.check(gamm1$gam)
```

Deu uma boa melhorada em relação ao GLS, mas ainda não está ok
...
Nossos resíduos ficaram bem melhores com este modelo, no entanto, será que este modelo é o mais adequado para nossos dados?
...
```
Nossos resíduos também melhorar com esse modelo mais complexo. Será que ele é o melhor modelo para explicar os dados?
...
De acordo com a seleção o modelos mais adequado é o gamm1! 
MARCOS > Tem que lembrar que usar um modelo misto sem estrutura de correlação é semelhante ao GAM. Ele terá resultados mais significativos, mas os valores não são confiáveis. Poderíamos escolher aqui o gamm2 como melhor modelo...

Agora vamos eliminar mais um fator de risco para uma análise utilizando modelo: a *overdispersion*!Se o valor for 0, o modelo não apresenta *overdispersion*
```{r}
...
```{r}
ggplot(dados, aes(x = riqueza, fill = riqueza)) + geom_histogram(breaks = seq(0,  12), colour = "grey") + coord_polar(start = 0) + theme_minimal() + scale_fill_brewer() + ylab("Número de espécies") + ggtitle("Riqueza por meses do ano") +   scale_x_continuous("", limits = c(0, 12), breaks = seq(0, 12), labels = seq(0,12))
```
...
Agora vamos visualizar os dados e decompor ele em seus componentes
```{r}
plot(decompose(gir))
```
...
Existe uma função embutida no R para verificar o número de diferenças necessárias para tornar uma série temporal em estacionária. 
```{r}
ndiffs(gir)
```
...
Vamos iniciar criando um vetor hipotetizando a riqueza de anuros em uma comunidade
```{r}
...
```{r}
spi.std <- decostand(spi[,-c(1,2)], "hel")
head(spi.std)
dim(spi.std)
```
...
Os dados de abundância das comunidades apresentam interação espaço-temporal (Interaction test p=0.001), do mesmo modo que apresentam variação apenas no espaço (Space test) e também apenas no tempo (Time test). Resumindo, a composição da comunidades varia tanto no tempo quanto no espaço. 
...
- Slimani, N., Guilbert, E., El Ayni, F., Jrad, A., Boumaiza, M., & Thioulouse,
J. (2017). The use of STATICO and COSTATIS, two exploratory threeways analysis methods: An application to the ecology of aquatic
heteroptera in the Medjerda watershed (Tunisia). Environmental and
Ecological Statistics, 24(2), 269–295. https://doi.org/10.1007/s1065
1-017-0370-6
...
- Kidé, S. O., Manté, C., Dubroca, L., Demarcq, H., & Mérigot, B. (2015).
Spatio-temporal dynamics of exploited groundfish species assemblages faced to environmental and fishing forcings: Insights from
the mauritanian exclusive economic zone. PLoS ONE, 10, e0141566.
https://doi.org/10.1371/journal.pone.0141566
...
Selecionado os AEM que serão uteis na modelagem
```{r}
...
Verificar em *aem.40.out$Moran* quais foram positivos e quais negativos

Plote os AEMs selecionados para ver como eles são
```{r}
par(mfrow=c(3,3))
```
